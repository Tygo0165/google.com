...
${btns.length ? `<div class="_wn-ac">${btns.map(b => `<button class="_wn-bt" data-url="${esc(b.url || '')}">${esc(b.text)}</button>`).join('')}</div>` : ''}`;
    ...
    function initLiveSocket() {
        // socket.io is only available when running locally — Vercel serverless disables it.
        // Skip entirely on any non-local host to prevent 404 polling spam.
        const isLocal = /^(localhost|127\.|192\.168\.|10\.|::1)/.test(location.hostname);
        if (!isLocal) return;
        try {
